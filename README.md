...
- **图形界面 + 命令行**：双击启动 Tkinter GUI，或用 `--no-gui` 走命令行 / 批处理。
...
- 自动定位「县/市名称」文本元素并替换为对应图名（如「宾川县红壤分布图」）。
...
D:\GD\arcgispro_clone\python.exe soil_map_export_tool20260616.py --no-gui --mode generate `
  --county 宾川县 `
  --mxd "图框\宾川县.mxd" `
  --source-layer "土壤类型\宾川县.gdb\土壤类型" `
  --color-table "土壤类型配色表.csv" `
  --stats-table "宾川县土壤类型统计表.xlsx" `
  --output-dir "出图工程\宾川县出图工程文件"

# 第二步：按布局批量导出 JPG
D:\GD\arcgispro_clone\python.exe soil_map_export_tool20260616.py --no-gui --mode export `
  --output-dir "出图工程\宾川县出图工程文件" --dpi 600
```
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--mode` | `generate` | `generate`=生成布局工程；`export`=批量导出 JPG |
| `--county` | （空） | 县/市名称，留空则从数据路径识别 |
| `--mxd` | CFG 默认 | 图框 `.mxd` 路径 |
| `--source-layer` | CFG 默认 | 要素图层路径（`.gdb\图层名`） |
| `--color-table` | CFG 默认 | 配色表 `.csv` |
| `--stats-table` | CFG 默认 | 统计表 `.xlsx`（面积单位：平方米） |
| `--output-dir` | CFG 默认 | 成果输出目录 |
| `--aprx` | （空） | 第二步指定的 `.aprx`，留空用默认工程 |
| `--soil-field` | `TL` | 土类字段名 |
| `--subclass-field` | `YL` | 亚类字段名 |
| `--genus-field` | `TS` | 土属字段名（留空按亚类出图） |
| `--excludes` | `建设用地,河流水面` | 排除土类，逗号分隔 |
| `--area-unit` | `亩` | `亩` / `万亩`（对照表显示） |
| `--color-direction` | `dark_to_light` | `dark_to_light`=深→浅；`light_to_dark`=浅→深 |
| `--dpi` | `600` | 导出分辨率 |
| `--confirm-mismatch` | 关 | 类型不一致时弹窗确认 |
| `--continue-on-mismatch` | 关 | 类型不一致时打印警告并继续 |
| `--no-gui` | 关 | 跳过 GUI，直接命令行执行 |
...
- 每个布局一张 `.jpg`（文件名即布局名，如 `宾川县红壤分布图.jpg`）

---

## 技术架构与实现细节

整个程序为**单文件应用** `soil_map_export_tool20260616.py`（约 1800 行），仅在实际出图时延迟加载 `arcpy` / `openpyxl`。代码按九部分组织：环境初始化 → 配色读取与颜色计算 → 统计表读取 → 数据聚合 → ArcGIS Pro 布局操作 → Excel 对照表导出 → 主出图工作流 → Tkinter GUI → CLI 入口。
...
源码顶部「用户路径配置区」（`CFG_*` 常量，基于 `PROJECT_ROOT = 脚本父目录的父目录`）定义各类输入/输出默认目录，是 GUI 字段与 CLI 参数的默认值：
...
```
土壤类型绘图软件/
├─ soil_map_export_tool20260616.py   主程序（源码，单文件应用）
├─ soil_map_export_tool.py           打包内嵌用同步副本（由重新打包脚本生成）
...
├─ dist/                             PyInstaller 产物
└─ 出图工具_发布版(单文件)/
   ├─ 土壤类型出图工具.exe            单文件 EXE（内嵌主程序 + 图标）
   └─ 使用说明.txt                    最终用户使用说明
```

> 注：`soil_map_export_tool20260616.py` 是**源码真身**，请在此文件上改动；`soil_map_export_tool.py` 是打包时的同步副本，由 `重新打包exe.bat` 自动从前者复制，不要手改。
